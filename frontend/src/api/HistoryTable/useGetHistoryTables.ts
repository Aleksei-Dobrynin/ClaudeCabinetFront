...
export const getHistoryTablesByApplication = (application_id: number, date_start: string, date_end: string): Promise<any> => {
  return http.get(`/api/v1/HistoryTable/GetByApplication?application_id=${application_id}&date_start=${date_start}&date_end=${date_end}`);
};
