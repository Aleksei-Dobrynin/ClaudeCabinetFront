...
class NewStore {
  ...
  loadHistoryTables = async () => {
    const date_start = this.date_start?.isValid() ? (this.date_start.toString()) : ""
    const date_end = this.date_end?.isValid() ? (this.date_end.toString()) : ""
    try {
      MainStore.changeLoader(true);
      const response = await getHistoryTablesByApplication(this.ApplicationID, date_start, date_end);
