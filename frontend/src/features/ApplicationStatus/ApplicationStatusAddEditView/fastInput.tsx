...
import storeList from "./../ApplicationStatusListView/store";
...
const FastInputView: FC<ApplicationStatusProps> = observer((props) => {
  ...
  useEffect(() => {
    if (props.mainId !== 0 && storeList.mainId !== props.mainId) {
      ...
      storeList.loadApplicationStatuss();
    }
  }, [props.mainId]);
  ...
  const columns = [
    {
                    ...
                    headerName: translate("label:application_statusListView.description_kg"),
                },
                {
                    field: 'text_color',
                    width: null, //or number from 1 to 12
                    headerName: translate("label:application_statusListView.text_color"),
                },
                {
                    field: 'background_color',
                    width: null, //or number from 1 to 12
                    headerName: translate("label:application_statusListView.background_color"),
                },
                {
                    field: 'name',
                    width: null, //or number from 1 to 12
                    headerName: translate("label:application_statusListView.name"),
                },
                {
                    field: 'description',
                    width: null, //or number from 1 to 12
                    headerName: translate("label:application_statusListView.description"),
                },
                {
                    field: 'code',
                    width: null, //or number from 1 to 12
                    headerName: translate("label:application_statusListView.code"),
                },
                {
                    field: 'created_at',
                    width: null, //or number from 1 to 12
                    headerName: translate("label:application_statusListView.created_at"),
                },
                {
                    field: 'updated_at',
                    width: null, //or number from 1 to 12
                    headerName: translate("label:application_statusListView.updated_at"),
                },
                {
                    field: 'created_by',
                    width: null, //or number from 1 to 12
                    headerName: translate("label:application_statusListView.created_by"),
                },
                {
                    field: 'updated_by',
                    width: null, //or number from 1 to 12
                    headerName: translate("label:application_statusListView.updated_by"),
                },
                {
                    field: 'name_kg',
                    width: null, //or number from 1 to 12
                    headerName: translate("label:application_statusListView.name_kg"),
                },
  ...
  return (
    <Container>
      <Card component={Paper} elevation={5}>
        <CardContent>
          <Box id="ApplicationStatus_TitleName" sx={{ m: 1 }}>
            <h3>{translate("label:ApplicationStatusAddEditView.entityTitle")}</h3>
          </Box>
          <Divider />
                    ...
                    {storeList.isEdit === false && (
                      <>
                        ...
                        <IconButton
                          ...
                          onClick={() => storeList.deleteApplicationStatus(entity.id)}
                        >
                          <DeleteIcon />
                        </IconButton>
                      </>
          ...
          {storeList.isEdit ? (
            ...
            <Grid item display={"flex"} justifyContent={"flex-end"} sx={{ mt: 2 }}>
              <CustomButton
                variant="contained"
                size="small"
                id="id_ApplicationStatusAddButton"
                onClick={() => {
                  storeList.setFastInputIsEdit(true);
                  store.doLoad(0);
