...
import { getDocumentStatus, createDocumentStatus, updateDocumentStatus } from "api/DocumentStatus";
import { DocumentStatus, DocumentStatusCreateModel } from "constants/DocumentStatus";
...
class DocumentStatusStore extends BaseStore {
  ...
  async onSaveClick(onSaved: (id: number) => void) {
    const data: DocumentStatusCreateModel = {
  ...
  async doLoad(id: number) {
    ...
    if (id) {
      this.id = id;
      await this.loadDocumentStatus(id);
    }
  }

  loadDocumentStatus = async (id: number) => {
    this.apiCall(
...
export default new DocumentStatusStore();
