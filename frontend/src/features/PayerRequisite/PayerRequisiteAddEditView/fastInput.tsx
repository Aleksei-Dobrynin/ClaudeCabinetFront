...
import storeList from "./../PayerRequisiteListView/store";
import CreateIcon from "@mui/icons-material/Create";
import DeleteIcon from "@mui/icons-material/Delete";
import CustomButton from "components/Button";

type PayerRequisiteProps = {
  children?: React.ReactNode;
  isPopup?: boolean;
  mainId: number;
};

const FastInputView: FC<PayerRequisiteProps> = observer((props) => {
  ...
  const columns = [
    {
                    ...
                    headerName: translate("label:payer_requisiteListView.payment_account"),
                },
                {
                    field: 'bank',
                    width: null, //or number from 1 to 12
                    headerName: translate("label:payer_requisiteListView.bank"),
                },
                {
                    field: 'bik',
                    width: null, //or number from 1 to 12
                    headerName: translate("label:payer_requisiteListView.bik"),
                },
                {
                    field: 'payer_idNavName',
                    width: null, //or number from 1 to 12
                    headerName: translate("label:payer_requisiteListView.payer_id"),
                },
  ...
  return (
    <Container>
      <Card component={Paper} elevation={5}>
        <CardContent>
          <Box id="PayerRequisite_TitleName" sx={{ m: 1 }}>
            <h3>{translate("label:PayerRequisiteAddEditView.entityTitle")}</h3>
          </Box>
          <Divider />
                    ...
                    {storeList.isEdit === false && (
                      <>
                        ...
                        <IconButton
                          ...
                          onClick={() => storeList.deletePayerRequisite(entity.id)}
                        >
                          <DeleteIcon />
                        </IconButton>
                      </>
            ...
            <Grid container spacing={3} sx={{ mt: 2 }}>
              ...
              <Grid item xs={12} display={"flex"} justifyContent={"flex-end"}>
                <CustomButton
                  variant="contained"
                  size="small"
                  id="id_PayerRequisiteSaveButton"
                  sx={{ mr: 1 }}
                  onClick={() => {
                ...
                <CustomButton
                  variant="contained"
                  size="small"
                  id="id_PayerRequisiteCancelButton"
                  onClick={() => {
                    storeList.setFastInputIsEdit(false);
                    store.clearStore();
                  }}
