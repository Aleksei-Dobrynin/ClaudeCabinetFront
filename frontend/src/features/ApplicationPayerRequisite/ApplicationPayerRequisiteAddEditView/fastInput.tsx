...
import storeList from "./../ApplicationPayerRequisiteListView/store";
import CreateIcon from "@mui/icons-material/Create";
import DeleteIcon from "@mui/icons-material/Delete";
import CustomButton from "components/Button";

type ApplicationPayerRequisiteProps = {
  children?: React.ReactNode;
  isPopup?: boolean;
  mainId: number;
};

const FastInputView: FC<ApplicationPayerRequisiteProps> = observer((props) => {
  ...
  useEffect(() => {
    if (props.mainId !== 0 && storeList.mainId !== props.mainId) {
      ...
      storeList.loadApplicationPayerRequisites();
    }
  }, [props.mainId]);
  ...
  const columns = [
    {
                    ...
                    headerName: translate("label:application_payer_requisiteListView.payment_account"),
                },
                {
                    field: 'bank',
                    width: null, //or number from 1 to 12
                    headerName: translate("label:application_payer_requisiteListView.bank"),
                },
                {
                    field: 'bik',
                    width: null, //or number from 1 to 12
                    headerName: translate("label:application_payer_requisiteListView.bik"),
                },
                {
                    field: 'application_payer_idNavName',
                    width: null, //or number from 1 to 12
                    headerName: translate("label:application_payer_requisiteListView.application_payer_id"),
                },
  ...
  return (
    <Container>
      <Card component={Paper} elevation={5}>
        <CardContent>
          <Box id="ApplicationPayerRequisite_TitleName" sx={{ m: 1 }}>
            <h3>{translate("label:ApplicationPayerRequisiteAddEditView.entityTitle")}</h3>
          </Box>
          <Divider />
                    ...
                    {storeList.isEdit === false && (
                      <>
                        ...
                        <IconButton
                          ...
                          onClick={() => storeList.deleteApplicationPayerRequisite(entity.id)}
                        >
                          <DeleteIcon />
                        </IconButton>
                      </>
          ...
          {storeList.isEdit ? (
            ...
            <Grid item display={"flex"} justifyContent={"flex-end"} sx={{ mt: 2 }}>
              <CustomButton
                variant="contained"
                size="small"
                id="id_ApplicationPayerRequisiteAddButton"
                onClick={() => {
                  storeList.setFastInputIsEdit(true);
                  store.doLoad(0);
