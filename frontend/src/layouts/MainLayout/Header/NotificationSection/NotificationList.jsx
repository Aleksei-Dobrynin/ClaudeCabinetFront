  ...
  return (
    ...
    >
      {store.notifications.map(notification => {
        ...
        let timeFromNow = `${diffMinutes} минут назад`;
        if (diffMinutes > 60) {
          timeFromNow = `${diffHours} часов назад`;
        }
        if (diffHours > 24) {
          timeFromNow = `${diffDays} дней назад`;
        }
        return <>
          <ListItemWrapper>
            ...
            <Grid container direction="column" className="list-container">
              ...
              <Grid item xs={12}>
                <Grid container justifyContent={"space-between"}>
                  {notification.link ? <Grid item>
                    <Chip label={translate("go")} sx={chipSuccessSX} onClick={() => {
                      store.clearNotify(notification.id)
                      window.location.href = notification.link
                    }} />
                  </Grid> : <span></span>}
                  <Grid item>
                    <Chip label={translate("clearNotify")} sx={chipWarningSX} onClick={() => {
                      store.clearNotify(notification.id)
                    }} />
                  </Grid>
                </Grid>
